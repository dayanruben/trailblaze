...
fun ToolPaletteDialog(
  /**
   * Loads the list of tools available for the current `(target, driver)` combination. The
   * JVM caller (desktop recording tab) passes `{ discoverAvailableTools(deviceManager, driverType) }`;
   * the wasmJs caller (web `/devices` viewer) passes `{ client.getToolCatalog(driver)?.tools ?: emptyList() }`.
   * Lifting this into a callback is what lets the same dialog compose on both targets — the
   * dialog has no compile-time dependency on `TrailblazeDeviceManager` anymore.
   */
  loadTools: suspend () -> List<TrailblazeToolDescriptor>,
  /**
  ...
  // Key on Unit, NOT loadTools. Callers pass `loadTools` as an inline lambda
  // (`{ discoverAvailableTools(...) }` / `{ client.getToolCatalog(...) }`), so the function
  // reference changes on every recomposition — keying on it caused the effect to cancel and
  // restart on every keystroke in the search field, hammering the daemon's tool-discovery
  // RPC. The load only needs to run once per dialog instance; Unit gives us exactly that.
  LaunchedEffect(Unit) {
    loading = true
    loadError = null
    try {
