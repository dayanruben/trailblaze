...
declare module "@trailblaze/scripting" {
  interface TrailblazeToolMap {
    ...
    // during descriptor codegen. So for `tapOnElementBySelector` the generated descriptor advertises
    // NO selector arg, and THIS hand-curated `nodeSelector` typing is the authoritative scripted-
    // author surface for it. Keep it in sync with `TapOnByElementSelector.kt` by hand (per the Drift
