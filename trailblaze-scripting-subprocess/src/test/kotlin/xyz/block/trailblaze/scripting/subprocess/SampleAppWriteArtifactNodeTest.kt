...
/**
 * CI-runnable end-to-end exercise of `sampleapp_writeArtifact.js`.
 *
 ...
 *
 * This test runs `sampleapp_writeArtifact.js` directly via `node` (universally available on every
 ...
 */
class SampleAppWriteArtifactNodeTest {
  ...
  companion object {
     ...
     */
    private val sampleAppWriteArtifactExampleFile: File = run {
      val relative =
        "examples/android-sample-app/trails/config/trailmaps/sampleapp/tools/sampleapp_writeArtifact.js"
    ...
    @BeforeClass @JvmStatic fun setup() {
      ...
      println(
        "SampleAppWriteArtifactNodeTest: setup running, " +
          "cwd=${System.getProperty("user.dir")}, runtime=${JS_RUNTIME?.absolutePath ?: "<none>"}",
      )
      check(sampleAppWriteArtifactExampleFile.isFile) {
        "sampleapp_writeArtifact.js not found via walk-up from cwd=${System.getProperty("user.dir")}; " +
          "expected at examples/android-sample-app/trails/config/trailmaps/sampleapp/tools/sampleapp_writeArtifact.js"
      }
      workDir = Files.createTempDirectory("sampleapp_writeArtifact_node_test_").toFile()
      runnerFile = File(workDir, "runner.mjs").apply {
        writeText(
          """
          // Test runner that invokes sampleapp_writeArtifact() once and prints a JSON envelope.
          //
          // Args (positional, all required):
          //   relativePath, contents, sessionId
          import { sampleapp_writeArtifact } from ${esmStringLiteral(sampleAppWriteArtifactExampleFile.absolutePath)};

          const [relativePath, contents, sessionId] = process.argv.slice(2);
          try {
            const summary = await sampleapp_writeArtifact(
  ...
  @Test fun `writes utf8 contents and reports correct byte count for multibyte chars`() {
    val sessionId = "sampleapp_writeArtifact_test_${System.nanoTime()}"
    ...
    try {
      ...
      val summary = checkNotNull(result.summary) {
        "sampleapp_writeArtifact returned no summary; error=${result.error}"
      }
  ...
  @Test fun `rejects path traversal via relativePath`() {
    val result = runRunner(
      ...
      contents = "x",
      sessionId = "sampleapp_writeArtifact_traversal_${System.nanoTime()}",
    )
  ...
  private fun runRunner(relativePath: String, contents: String, sessionId: String): RunnerResult {
    val runtime = checkNotNull(JS_RUNTIME) {
      "No JS runtime found. SampleAppWriteArtifactNodeTest requires `node`, `bun`, or `tsx` on " +
