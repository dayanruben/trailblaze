...
class WaypointShortcutVerifyCommand : Callable<Int> {
  ...
  private fun buildTrailYaml(
    ...
    // those values appear (yamlQuote-wrapped) on the `waypoint:` arg of the load-bearing
    // `assertWaypoint` tools below. Folding them into a free-text `step:` scalar too would
    // mean every interpolation site has to be wrapped, with no benefit beyond duplicating
