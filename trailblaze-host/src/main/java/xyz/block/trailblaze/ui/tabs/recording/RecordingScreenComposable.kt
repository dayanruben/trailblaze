...
import xyz.block.trailblaze.ui.recording.ActionYamlCard
...
import xyz.block.trailblaze.ui.recording.TrailheadPickerMode
...
fun RecordingScreenComposable(
  ...
  Row(Modifier.fillMaxSize()) {
    ...
    Column(
      ...
      if (paletteOpen) {
        ToolPaletteDialog(
          loadTools = { discoverAvailableTools(deviceManager, driverType) },
        ...
        Row(modifier = Modifier.fillMaxWidth().weight(1f)) {
          ...
          itemsIndexed(
            ...
            Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
              ...
              // Descriptor resolution lives in `RichEditorSupport` (jvm-only — needs
              // JVM reflection through `TrailblazeJsonInstance`). Passed in here so the same
              // commonMain `ActionYamlCard` can be reused by the wasmJs `/devices` recording
              // panel with `{ _ -> null }` (no rich-form editor on web yet — YAML-only).
              descriptorResolver = ::resolveDescriptorAndValues,
              )
              InsertHerePlus(onClick = {
