...
class HostOnDeviceRpcTrailblazeAgentTest {
  ...
  // ── Per-replayed-step screenshot for the native on-device accessibility replay report ──────
  //
  // On the host-RPC accessibility replay path a replayed selector tool runs on-device via
  // `executeNodeSelector*`, which emits screenshot-bearing `AgentDriverLog`s on the DEVICE but no
  // host-visible log — the host's catch-all emit is a screenshot-less `TrailblazeToolLog`. So the
  // report's session dir gets zero per-step images and its timeline reads "No screenshots yet".
  // `TrailblazeHostYamlRunner.perReplayedToolScreenshotHook` closes that gap: it captures one
  // host-side screenshot per replayed tool (via the same `captureScreenState` RPC the
  // screenshotProvider uses) and logs it as a screenshot-bearing `TrailblazeSnapshotLog`. These
  // tests pin that contract against the on-device server stub.

  /** Minimal valid 1x1 PNG, base64-encoded — what the device returns as the screenshot. */
  private val onePixelPngBase64 =
    "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+M8AAAMBAQDJ/pLvAAAAAElFTkSuQmCC"

  @Test
  fun `replayed tool produces a screenshot-bearing log via the per-step hook`() {
    // The device returns a screenshot from GetScreenState; the per-step hook must capture it and
    // emit a `TrailblazeSnapshotLog` whose `screenshotFile` is non-null — i.e. the report would
    // have >= 1 image for this replayed step.
    mockServer.onPost("/rpc/GetScreenStateRequest") {
      HttpStatusCode.OK to
        """{"viewHierarchy":{},"screenshotBase64":"$onePixelPngBase64","deviceWidth":1080,"deviceHeight":1920}"""
    }

    val captured = mutableListOf<TrailblazeLog>()
    val agent = createAgent()
    val logger = capturingLogger(captured)
    val session = TrailblazeSession(sessionId = SessionId("test-session"), startTime = Clock.System.now())

    val hook = TrailblazeHostYamlRunner.perReplayedToolScreenshotHook(
      sessionProvider = { session },
      captureScreenState = { agent.captureScreenState() },
      logSnapshot = { s, screen, displayName -> logger.logSnapshot(s, screen, displayName) },
    )

    runBlocking {
      hook(InputTextTrailblazeTool(text = "irrelevant"))
    }

    val snapshots = captured.filterIsInstance<TrailblazeLog.TrailblazeSnapshotLog>()
    assertThat(snapshots).hasSize(1)
    // HasScreenshot with a non-null screenshotFile is what the report's keyframe timeline reads.
    assertThat(snapshots.single().screenshotFile.isNotEmpty()).isEqualTo(true)
  }

  @Test
  fun `per-step hook emits nothing when the device returns no screenshot`() {
    // Negative control: a screenshot-less GetScreenState response must NOT yield a
    // `TrailblazeSnapshotLog` — proving the positive test asserts on a real, screenshot-bearing
    // log rather than passing vacuously.
    mockServer.onPost("/rpc/GetScreenStateRequest") {
      HttpStatusCode.OK to
        """{"viewHierarchy":{},"screenshotBase64":null,"deviceWidth":1080,"deviceHeight":1920}"""
    }
    ...
    assertThat(captured.filterIsInstance<TrailblazeLog.TrailblazeSnapshotLog>()).isEmpty()
  }

  /** PNG magic-number header — a real, non-empty screenshot. */
  ...
  private fun screenStateWith(bytes: ByteArray?): ScreenState = object : ScreenState {
    ...
    override val viewHierarchy: xyz.block.trailblaze.api.ViewHierarchyTreeNode =
      xyz.block.trailblaze.api.ViewHierarchyTreeNode(nodeId = 1, className = "FrameLayout")
    override val trailblazeDevicePlatform: TrailblazeDevicePlatform = TrailblazeDevicePlatform.ANDROID
    override val deviceClassifiers: List<xyz.block.trailblaze.devices.TrailblazeDeviceClassifier> = emptyList()
  }

  @Test
  fun `per-step hook retries once on empty bytes and recovers a real frame`() {
    // A transient framebuffer failure yields empty bytes on the first capture; the next frame is
    // healthy. The hook must retry once and log the recovered real screenshot — converting a
    // would-be blank keyframe into a real one.
    val captured = mutableListOf<TrailblazeLog>()
    val logger = capturingLogger(captured)
    val session = TrailblazeSession(sessionId = SessionId("test-session"), startTime = Clock.System.now())
    val attempts = AtomicInteger(0)
    ...
    val hook = TrailblazeHostYamlRunner.perReplayedToolScreenshotHook(
      sessionProvider = { session },
      captureScreenState = {
        if (attempts.getAndIncrement() == 0) screenStateWith(ByteArray(0)) else screenStateWith(pngHeaderBytes)
      },
      logSnapshot = { s, screen, displayName -> logger.logSnapshot(s, screen, displayName) },
    )

    runBlocking { hook(InputTextTrailblazeTool(text = "irrelevant")) }
  ...
  @Test
  fun `per-step hook emits nothing when both captures return empty bytes`() {
    // Honest limit: a persistently-degraded device returns empty bytes on both the first capture
    // and the single retry. The hook must skip cleanly — no 0-byte file, no screenshot-less log.
    val captured = mutableListOf<TrailblazeLog>()
    val logger = capturingLogger(captured)
    val session = TrailblazeSession(sessionId = SessionId("test-session"), startTime = Clock.System.now())
    val attempts = AtomicInteger(0)
    ...
    val hook = TrailblazeHostYamlRunner.perReplayedToolScreenshotHook(
      sessionProvider = { session },
      captureScreenState = { attempts.incrementAndGet(); screenStateWith(ByteArray(0)) },
      logSnapshot = { s, screen, displayName -> logger.logSnapshot(s, screen, displayName) },
    )
