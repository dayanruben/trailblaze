...
/**
 * Minimal `@Serializable` view of the trailmap manifest YAML — only the fields the bundler
 * actually reads. The authoritative shape lives in `:trailblaze-models`
 * (`TrailblazeTrailmapManifest`, `TrailmapTargetConfig`, `TrailmapScriptedToolFile`,
 ...
 */
@Serializable
internal data class BundlerTrailmapManifest(
  @SerialName("id") val id: String,
  @SerialName("dependencies") val dependencies: List<String> = emptyList(),
  @SerialName("target") val target: BundlerTarget? = null,
  /**
   * Top-level per-platform configuration. Library trailmaps (no `target:` block) declare
   * the runtime-registry needs of their internal scripted tools here. See the
   * authoritative shape on `TrailblazeTrailmapManifest.platforms` in `:trailblaze-models`.
   *
   * The bundler decodes this so per-trailmap typed-surface codegen can read it without
  ...
  /**
   * Tool **names** (not file paths) this trailmap publishes to consumers. Mirrors
   * `TrailblazeTrailmapManifest.exports` in `:trailblaze-models`. Tools authored under the
   * trailmap's `tools/` directory not listed here are internal helpers, invisible to
...
internal data class BundlerTarget(
   ...
   * `TrailmapTargetConfig.tools` in `:trailblaze-models`. Each entry must match the `name:`
   * field on a descriptor auto-discovered under `<trailmap>/tools/`. The bundler's discovery
   * walk in [TrailblazeTrailmapBundler] resolves each name against that registry.
   */
...
internal data class BundlerToolFile(
   ...
   * authoritative shape on `:trailblaze-models`' `TrailmapScriptedToolFile.script` is
   ...
   * [TrailblazeTrailmapBundleException.JsToolFileNotAllowed]).
   */
   ...
   * export on the shared `script:` source. Mirrors `TrailmapScriptedToolFile.tools` in
