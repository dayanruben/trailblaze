...
// view of the trailmap-manifest YAML (see `BundlerTrailmapManifest`) rather than depending on
...
//      includedBuild and can't take a `project(":trailblaze-trailmap-bundler")` dep directly.
...
//      module via standard `implementation(project(":trailblaze-trailmap-bundler"))`.

dependencies {
  implementation(libs.kaml)
  implementation(libs.kotlinx.serialization.core)

  // [WorkspaceClientDtsGenerator] takes `ToolDescriptor` and `TrailmapScriptedToolFile` directly
  ...
  // Tests construct real `ToolDescriptor` and `TrailmapScriptedToolFile` instances, so the test
