...
abstract class BundleAuthorToolsTask : DefaultTask() {
  ...
  fun bundle() {
    ...
    // Synthesize the registration wrapper in the (gitignored) build OUTPUT dir — NOT co-located in
    // the author's source `tools/` dir. Co-locating it there raced with AGP's `mergeDebugAssets`,
    // which hashes the trails-config asset source folders: a concurrent bundle task's transient
    // wrapper would be deleted mid-snapshot and fail the build with NoSuchFileException (seen across
    // the ios-pr + mandatory-checks pipelines once recursive discovery raised the bundle-task count).
    // The wrapper imports the entry by ABSOLUTE path, so esbuild resolves it from anywhere; the
    // entry's own relative imports still resolve against its real dir. Same build-dir placement the
    // stdio stub below already uses.
    val wrapperFile = File.createTempFile(".trailblaze-wrapper-", ".ts", output.parentFile)
    wrapperFile.writeText(
      synthesizeInProcessScriptedToolWrapper(entry.absolutePath, entry.name, wrapperTemplate),
    )
 ...
 * token substitution.
 */
// [importSource] is the module specifier the synthesized wrapper imports the user's entry from —
// caller-controlled so the wrapper can live OUTSIDE the author's source tree (it's written to the
// gitignored build dir, then imports the entry by absolute path). Co-locating the wrapper in the
// source `tools/` dir caused a Gradle/AGP input-snapshot race: a concurrent `mergeDebugAssets`
// hashing that dir would fail with NoSuchFileException when the transient wrapper got deleted
// mid-snapshot. [displayName] is only for the human-readable header comment.
fun synthesizeInProcessScriptedToolWrapper(importSource: String, displayName: String, templateFile: File): String {
  val header = buildString {
    appendLine("// Synthetic entry generated by BundleAuthorToolsTask.")
    appendLine("// Imports every typed-tool export from `$displayName`, builds a `client`")
