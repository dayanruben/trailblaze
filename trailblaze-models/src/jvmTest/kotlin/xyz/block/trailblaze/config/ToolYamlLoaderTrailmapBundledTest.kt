...
/**
 * Pins the trailmap-bundled tool discovery added alongside the library/target trailmap
 * distinction. Tools that ship inside a trailmap at
 * `trails/config/trailmaps/<id>/{tools,shortcuts,trailheads}/<name>.<kind>.yaml`
 * must surface in the same global tool registry that the flat
 * `trails/config/tools/` scan populates so toolset YAMLs and trail YAMLs can
 * reference them by bare id regardless of which trailmap ships them. Without this
 * hook, moving a tool YAML out of the flat dir into a trailmap subdirectory would
 ...
 *  - `<trailmap>/tools/<name>.tool.yaml`
 *  - `<trailmap>/shortcuts/<name>.shortcut.yaml`
 *  - `<trailmap>/trailheads/<name>.trailhead.yaml`
 *
 ...
 */
class ToolYamlLoaderTrailmapBundledTest {
  ...
  @Test
  fun `trailmap-bundled class-mode tool surfaces in discoverAndLoadAll`() {
    ...
    val root = newTempDir()
    val trailmapToolsDir = File(root, "trails/config/trailmaps/sample-library/tools").apply { mkdirs() }
    File(trailmapToolsDir, "sampleLibrary_saveItem.tool.yaml").writeText(
      """
    ...
    withClasspathRoot(root) {
      ...
      assertTrue(
        ...
        "Expected trailmap-bundled tool to surface in global registry, got: ${resolved.keys}",
      )
    }
  }

  @Test
  fun `trailmap-bundled shortcut tool surfaces in discoverYamlDefinedTools when tools-mode`() {
    val root = newTempDir()
    val trailmapShortcutsDir = File(root, "trails/config/trailmaps/sample/shortcuts").apply { mkdirs() }
    File(trailmapShortcutsDir, "go_to_home.shortcut.yaml").writeText(
      """
      id: go_to_home
    ...
    withClasspathRoot(root) {
      ...
      assertTrue(
        ...
        "Expected trailmap-bundled shortcut tool to surface, got: ${resolved.keys}",
      )
    }
  }

  @Test
  fun `trailmap-bundled scripted-tool YAML with bare yaml suffix is NOT picked up`() {
    // TrailmapScriptedToolFile descriptors live in `<trailmap>/tools/<name>.yaml` (no operational
    ...
    // trailmap-style scripted tool doesn't accidentally register as a class-backed tool.
    val root = newTempDir()
    val trailmapToolsDir = File(root, "trails/config/trailmaps/clock/tools").apply { mkdirs() }
    File(trailmapToolsDir, "clock_android_launchApp.yaml").writeText(
      """
  ...
  @Test
  fun `trailmap-bundled YAML in unrecognized top-level subdirectory is NOT picked up as a tool`() {
    // The convention layout pins each operational class to a single top-level dir:
    // `tools/`, `shortcuts/`, `trailheads/`. A YAML at some other path inside the trailmap
    // tree (e.g. a misplaced waypoint file with a `.tool.yaml` suffix) must not register
    // as a tool — discovery is intentionally narrow.
    val root = newTempDir()
    val miscDir = File(root, "trails/config/trailmaps/sample/misc").apply { mkdirs() }
  ...
  @Test
  fun `trailmap-bundled shortcut YAML under tools dir is NOT picked up`() {
  ...
  @Test
  fun `trailmap-bundled YAML nested under tools subdirectory IS picked up`() {
    // The layout is `<trailmap-id>/tools/[<subdir>/...]<name>.tool.yaml` — any depth under
    // `tools/` is acceptable so authors can organize a trailmap's tool surface (e.g. by
    ...
    val root = newTempDir()
    val nestedDir = File(root, "trails/config/trailmaps/sample/tools/subdir").apply { mkdirs() }
  ...
  @Test
  fun `trailmap-bundled YAML at multiple depths under tools subdirectory all register`() {
    // Pin: depth is unrestricted under each top-level dir. A trailmap can use shallow
    // (tools/web/, shortcuts/web/) and deep (tools/android/checkout/keypad/) nesting in
    // the same trailmap — subdirs are organizational only.
    val root = newTempDir()
    val webDir = File(root, "trails/config/trailmaps/sample/tools/web").apply { mkdirs() }
    ...
    val deepDir = File(root, "trails/config/trailmaps/sample/tools/android/checkout/keypad")
      .apply { mkdirs() }
  ...
  @Test
  fun `trailmap-bundled shortcuts and trailheads in their own dirs surface alongside tools`() {
    // Pin the cross-class layout: a single trailmap with all three operational classes,
    // each in its own top-level dir, all register together.
    val root = newTempDir()
    val trailmapDir = File(root, "trails/config/trailmaps/multi").apply { mkdirs() }
    ...
    val shortcutsDir = File(trailmapDir, "shortcuts/web").apply { mkdirs() }
  ...
  fun `same-basename tools in two different trailmaps both register without collision`() {
    ...
    val root = newTempDir()
    val trailmapAToolsDir = File(root, "trails/config/trailmaps/trailmap-a/tools").apply { mkdirs() }
    File(trailmapAToolsDir, "shared.tool.yaml").writeText(
      """
      id: pack_a_shared
      class: xyz.block.trailblaze.config.FakeToolYamlLoaderTool
      """.trimIndent(),
    )
    val trailmapBToolsDir = File(root, "trails/config/trailmaps/trailmap-b/tools").apply { mkdirs() }
    File(trailmapBToolsDir, "shared.tool.yaml").writeText(
      """
      id: pack_b_shared
      class: xyz.block.trailblaze.config.FakeToolYamlLoaderTool
      """.trimIndent(),
    ...
    withClasspathRoot(root) {
      ...
      assertTrue(
        ...
        "Expected both same-basename trailmap tools to surface, got: ${resolved.keys}",
      )
    }
  }

  @Test
  fun `library trailmap trailhead tool is skipped at discovery time`() {
    // Pin: even if a library trailmap (no `target:` in trailmap.yaml) physically ships a
    // `*.trailhead.yaml` file under its `trailheads/` directory — i.e. the manifest-side
    // guard in `TrailblazeProjectConfigLoader.resolveTrailmapSiblings` would never fire
    ...
    // and only make sense within a target trailmap.
    val root = newTempDir()
    val trailmapDir = File(root, "trails/config/trailmaps/library-only").apply { mkdirs() }
    File(trailmapDir, "trailmap.yaml").writeText(
      """
      id: library-only
      """.trimIndent(),
    ...
    withClasspathRoot(root) {
      ...
      assertEquals(
        emptyMap(),
        resolved,
        "Library-trailmap trailhead tool must not register at discovery; got: ${resolved.keys}",
      )
    }
  }

  @Test
  fun `target trailmap trailhead tool registers normally at discovery`() {
    // Symmetric happy-path: a target trailmap (target: present) legitimately owns trailhead
    // tools, so the discovery-time guard must not fire here.
    val root = newTempDir()
    val trailmapDir = File(root, "trails/config/trailmaps/target-trailmap").apply { mkdirs() }
    File(trailmapDir, "trailmap.yaml").writeText(
      """
      id: target-trailmap
      target:
        display_name: Target Trailmap
      """.trimIndent(),
    )
    val trailheadsDir = File(trailmapDir, "trailheads").apply { mkdirs() }
    File(trailheadsDir, "go_home.trailhead.yaml").writeText(
    ...
    withClasspathRoot(root) {
      ...
      assertTrue(
        ...
        "Expected target-trailmap trailhead tool to surface, got: ${resolved.keys}",
      )
    }
  }

  @Test
  fun `trailmap-bundled tools surface when discovery is routed through a non-classpath ConfigResourceSource`() {
    // Regression for the on-device Android registration gap: pre-fix, discoverTrailmapBundledToolContents
    ...
    // trailmap-bundled hook as a no-op and tools were silently dropped.
    val source = object : ConfigResourceSource {
      ...
      override fun discoverAndLoadRecursive(directoryPath: String, suffix: String): Map<String, String> {
        if (directoryPath != "trails/config/trailmaps") return emptyMap()
        val trailmapYaml = """
          id: target-trailmap
          target:
            display_name: Target Trailmap
        ...
        """.trimIndent()
        return when (suffix) {
          "/trailmap.yaml" -> mapOf("target-trailmap/trailmap.yaml" to trailmapYaml)
          ".tool.yaml" -> mapOf("target-trailmap/tools/in_memory_tool.tool.yaml" to toolYaml)
          else -> emptyMap()
        }
      }
    }
    ...
    assertTrue(
      ...
      "Expected trailmap-bundled tool from injected source to surface; got: ${resolved.keys}",
    )
  }

  private fun newTempDir(): File =
    createTempDirectory("tool-yaml-loader-trailmap-bundled-test").toFile().also { tempDirs += it }
