...
class CliDocsGenerator(
  ...
  private fun generateCliPage(commandLine: CommandLine) {
    ...
    val content = buildString {
      // Generator banner — manual edits to this file are silently overwritten by
      // the next `:docs:generator:run` invocation (CI runs it via
      // `scripts/generate-docs-and-diff.sh` and asserts no diff). Doc changes
      // belong on the picocli `@Option` / `@Parameters` / `@Command` annotations,
      // not here. The banner stays out of MkDocs render via the standard `<!-- -->`
      // HTML comment idiom — markdown viewers ignore it.
      appendLine("<!-- GENERATED FILE — DO NOT EDIT BY HAND. -->")
      appendLine("<!-- Generated by :docs:generator:run from picocli annotations. -->")
      appendLine("<!-- Edit the @Option / @Parameters / @Command descriptions on the source classes, then re-run. -->")
      appendLine()
