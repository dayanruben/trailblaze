...
- **Android (Accessibility, Compose) and iOS (AXe, XCUITest, Maestro): thickest.** This is where the framework does the most work of its own. The native frameworks have gaps we have to fill — cross-driver view-hierarchy capture, simulator-vs-device performance differences, selector-resolution semantics that diverge from native, etc. The shim here is not thin; the discipline is to keep the divergences additive (we never replace a native primitive with a worse Trailblaze one) and to expose native primitives as passthrough tools wherever possible so users can always escape down to the framework.

Stated honestly: Playwright is already feature-complete for our purposes, so on web this is genuinely a shim. On mobile the underlying frameworks have gaps, and we fill them with as little as we can — view-hierarchy stack, perf optimizations, cross-driver normalization — while exposing the native primitives as escape-hatch tools so the discipline still applies to user-facing code.
...
## Where code generation actually lives

Two separate claims, easy to conflate:

- **What ships today.** On web, `--export-playwright` emits a runnable Playwright spec from any recorded trail. It works because the web driver is a thin shim over Playwright, so the recorded trail and the underlying Playwright actions coincide one-to-one on that driver.
- **What the architecture allows in general.** Code generation isn't a property of the authoring layer; it's a property of the execution trace. Every action a Trailblaze trail performs resolves to a native-driver call. Those resolved calls can be captured and rendered as Playwright TypeScript, Espresso Kotlin, XCUITest Swift, or any other native dialect *without* requiring the authoring surface (trailheads, scripted tools, waypoint navigation, custom domain tools) to be expressible in that dialect. Authoring is the source language; export is the target. They intentionally don't have the same expressiveness — that's the point.

The second claim is what lets the LLM-callable custom-tool surface and the native-framework-export surface coexist. Authoring sits above the framework level and export sits below it: the same execution-layer artifact a single-layer codegen tool would produce, plus an authoring layer that doesn't have to be expressible in the target dialect.

If only the first claim holds, the authoring abstraction has collapsed onto the framework it wraps — which is a real design choice, but it should be made deliberately rather than by drift.
