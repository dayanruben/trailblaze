...
The [Model Context Protocol (MCP)](https://modelcontextprotocol.io) provides a standardized interface for exposing Trailblaze capabilities to external AI systems. Trailblaze uses the **Streamable HTTP** transport, which allows MCP clients to connect via HTTP POST requests to a session-based endpoint.
