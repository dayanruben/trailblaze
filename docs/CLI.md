...
| `--capture-video` | Record device screen video for the session. Off by default — video writes large files — pass --capture-video to enable it for a run. When neither flag is passed, inherits TRAILBLAZE_CAPTURE_VIDEO and the saved `trailblaze config capture-video` setting. | - |
...
| `android-driver` | Android driver type | accessibility, in-process |
