...
* `-Dtrailblaze.callback.timeoutMs` (JVM system property, defaults to `30000`) — Per-callback dispatch timeout on the daemon side. Bounds how long a single `client.tools.<name>(args)` dispatch can run before the daemon returns a structured timeout error. Raise when a target tool is legitimately slow (e.g. waiting for a screen to settle on a slow emulator).
