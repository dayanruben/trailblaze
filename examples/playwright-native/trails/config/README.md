...
```bash
cd examples/playwright-native/trails/config/tools
bun install
```
...
**Path note for adopting this pattern in another trailmap:** the `file:` link in `package.json`
...
`file:../../../../sdks/typescript` (4 levels up: `tools/` → `trails/config/` →
`playwright-native/` → `examples/` → repo root, then down to `sdks/typescript`). Adjust
the depth to match your own trailmap's location before copying.
...
```bash
export TRAILBLAZE_CONFIG_DIR=$PWD/examples/playwright-native/trails/config
...
```yaml
tools:
  - script: ./trails/config/tools/myTarget_web_myTool.ts
...
export async function myTarget_web_myTool(
  ...
  await client.tools.web_verifyTextVisible({
...
`./trailblaze check playwrightSample` to validate the trailmap end-to-end; the test step
shells out to `bun test` against the mock client + mock context from
`@trailblaze/scripting/testing`, so tests run without a daemon or a device. Requires
`bun` on PATH (install from https://bun.sh). See the existing samples for patterns:
...
export async function playwrightSample_web_openFormIfNeeded(args, ctx, client) {
  try {
    await client.tools.web_verifyTextVisible({ text: "Contact Form" });
...
filesystem `trails/config/tools/*.yaml` script manifests are not wired into the runtime yet.
