...
//
//   This sample pack hosts the canonical "unit-test fixture" for the per-pack TS
...
//   a pack's perspective, importing anything from `@trailblaze/scripting` pulls the
//   bundle in — which makes the `declare global { ... }` block in the appendix
//   visible — so the expressions below compile cleanly without a per-pack
//   `globals.d.ts` shim. Without a fixture, an accidental shrink of
//   `runtime-globals.d.ts` would slip through unnoticed until a real pack hit the
//   missing global.
//
// CI WIRING:
//
//   Per PR #3219 (post-Phase-B), `pr_validate_ts_tooling.sh` discovers per-pack
//   tool dirs by the framework-emitted `tools/tsconfig.json` (not the pre-pack
//   `tools/package.json` it used to look for), so this file IS now CI-backed —
//   every PR runs `tsc --noEmit` against the pack's generated tsconfig and any
...
// block when the module is in the program. A pack's real tool file always imports
