...
//
// `playwrightSample_web_openFormIfNeeded` probes the page with `web_verify_text_visible`
...
describe("playwrightSample_web_openFormIfNeeded", () => {
  ...
  test("attempts the recovery click when the heading probe throws", async () => {
    ...
    // `web_verify_text_visible` invocations in the tool body will throw, which means
    ...
    client.stub("web_verify_text_visible", {
      textContent: "",
    ...
    expect(client.calls.map((c) => c.tool)).toEqual([
      "web_navigate",
      "web_verify_text_visible", // initial probe — thrown by the stub
      "web_click", // recovery branch reached
      "web_verify_text_visible", // retry — thrown by the same stub
    ]);
