// Sample test for a host-only scripted tool. `sampleapp_writeArtifact` reads and writes the
...
// `client.tools.X()` dispatch + stub patterns, see the playwright-native trailmap's sibling
// `.test.ts` files.
//
// Run via:
//
...
import { sampleapp_writeArtifact } from "./sampleapp_writeArtifact";
...
describe("sampleapp_writeArtifact", () => {
  test("writes the contents to the per-session tmpdir and returns the byte count", async () => {
    ...
    const result = await sampleapp_writeArtifact(
  ...
  test("applies module defaults when relativePath and contents are omitted", async () => {
    ...
    const result = await sampleapp_writeArtifact({}, ctx);
  ...
  // Both shapes that escape the per-session subtree share one diagnostic. The
  // `test.each` keeps "this rule rejects every escape attempt" on one screen and
  // adds a new attack shape with one row instead of a new test.
  test.each([
    ["absolute path", "/etc/passwd"],
    ["dotdot traversal inside an otherwise relative path", "nested/../../escape.txt"],
  ])("rejects %s rather than escaping the per-session subtree", async (label, relativePath) => {
    const ctx = createMockContext({
      platform: "android",
      sessionId: uniqueSessionId(`reject-${label.replace(/\W+/g, "-")}`),
    });
    await expect(
      sampleapp_writeArtifact({ relativePath, contents: "" }, ctx),
