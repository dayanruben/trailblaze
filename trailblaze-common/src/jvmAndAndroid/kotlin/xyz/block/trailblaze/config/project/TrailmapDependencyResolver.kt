...
/**
 * Walks a trailmap's dependency graph and applies closest-to-root-wins inheritance from each
 * dependency's `defaults:` map onto the consumer's `target.platforms`.
 *
 * ## Mental model
 *
 * Composition mirrors a Gradle classpath. A consumer trailmap declares `dependencies: [...]`;
 * resolution is transitive; each trailmap contributes its `defaults` (per-platform field
 * defaults) to dependers. The framework ships a single `trailblaze` trailmap that publishes
 * the standard per-platform `defaults:`. Built-in toolsets themselves (`core_interaction`,
 * `web_core`, etc.) ship alongside their owning trailmaps at
 * `trails/config/trailmaps/<id>/toolsets/<name>.yaml` and surface globally through the
 * toolset loader — the `trailblaze` trailmap only references them by id from its
 * `defaults:`. The conventional consumer preamble is `dependencies: [trailblaze]`.
 *
 * ## Resolution rules
 *
 ...
 *   are expected to catch and skip the consumer trailmap with a logged error so sibling
 *   trailmaps continue to load.
 *
 * ## Implementation: collect-then-merge by depth
 *
 * The walk records every contributing trailmap as a [DefaultsContribution] tagged with its
 ...
 *
 * The framework `trailblaze` trailmap is auto-discovered via JVM classpath only — see
 * [TrailblazeTrailmapManifestLoader.discoverAndLoadFromClasspath]. On-device builds (the
 * Android on-device MCP server) don't run that discovery, so a consumer trailmap with
 * `dependencies: [trailblaze]` will fail dep resolution on-device, get caught by the
 * loader's atomic-per-trailmap handler, and be silently skipped. This is intentional today
 * (trailmap discovery is a JVM-host CLI concern); on-device trailmap discovery is out of scope
 * until there's a real requirement for it.
 *
 * ## Parity contract with build-logic
 *
 * `build-logic/src/main/kotlin/TrailblazeBundledConfigTasks.kt`'s `TrailmapTargetGenerator`
 * (specifically [`mergeInheritedDefaults`](../../../../../../../../build-logic/src/main/kotlin/TrailblazeBundledConfigTasks.kt))
 * implements the SAME closest-wins semantics described above against trailmap manifests
 ...
 * carry a "behavioral contract" test (`TrailmapDependencyResolverTest` here,
 * `TrailmapTargetGeneratorTest` in build-logic) that pins the same set of cases — if either
 * test gets modified, the other should be examined too.
 */
internal object TrailmapDependencyResolver {

  /**
   * Returns [ownTarget] with `platforms` field-level enriched by closest-wins defaults
   * walked from [ownDependencies] through [trailmapsById]. Throws when [ownDependencies]
   * references an unknown trailmap or the graph contains a cycle.
   */
  fun resolveTarget(
    ...
    val visiting = mutableSetOf(rootTrailmapId)
  ...
  /**
   * DFS walk that records each trailmap's `defaults:` (when present) as a
   ...
   * it with the root trailmap id so a self-referential `dependencies: [<root>]` declaration
   * is caught.
   *
   ...
   * sequence in which DFS encounters each contributing trailmap. The merge step
   ...
   * correct under multi-path graphs. At current trailmap counts (single-digit depth, low
  ...
  private fun walk(
    ...
    if (depId in visiting) {
      throw TrailblazeProjectConfigException(
        "Cycle detected in trailmap dependencies involving '$depId' " +
          "(chain: ${visiting.joinToString(" -> ")} -> $depId)",
      )
    }
    val depTrailmap = trailmapsById[depId]
      ?: throw TrailblazeProjectConfigException(
        "Trailmap '$depId' not found " +
          "(declared in 'dependencies:' chain: ${visiting.joinToString(" -> ")} -> $depId)",
      )
    visiting += depId
    depTrailmap.manifest.defaults?.let { defaults ->
    ...
    depTrailmap.manifest.dependencies.forEach { childDep ->
      walk(childDep, depth + 1, trailmapsById, visiting, contributions)
    }
    visiting -= depId
  }
  ...
  /**
   * One trailmap's contribution to the resolved defaults pool, paired with the [depth] from
   * the consuming trailmap and the global DFS [encounterOrder]. The merge step uses these
