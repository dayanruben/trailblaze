...
object TrailblazeCompiler {
  ...
  fun compile(
    ...
    val referenceErrors = collectUnresolvedReferences(resolved, referenceSource, commandLabel)
  ...
  private fun deleteOrphanOutputs(outputDir: File, keepNames: Set<String>): List<File> {
    ...
    for (file in candidates) {
      ...
      // Recognize both the current banner and the legacy `trailblaze compile` banner
      ...
      if ((firstLine == GENERATED_BANNER || firstLine in LEGACY_GENERATED_BANNERS) && file.delete()) {
        deleted += file
      }
    }
    return deleted
  ...
  /**
   * Banners written by previous versions of this compiler. [deleteOrphanOutputs]
   * treats files starting with any of these as compiler-owned for cleanup purposes,
   * so renaming the command surface (e.g. the `trailblaze compile` → `trailblaze
   * check` unification) doesn't strand stale targets in users' working trees.
   */
