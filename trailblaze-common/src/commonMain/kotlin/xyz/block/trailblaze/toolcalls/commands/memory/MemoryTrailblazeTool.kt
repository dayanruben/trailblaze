 ...
 *
 * Not sealed: implementations may live in narrower source sets than this interface
 * (e.g. `AssertMathTrailblazeTool` stays JVM/Android-only for its exp4j dependency), and
 * a sealed supertype in commonMain cannot be extended from a sibling source set.
 */
interface MemoryTrailblazeTool : TrailblazeTool, ReadOnlyTrailblazeTool {
  fun execute(
    memory: AgentMemory,
